...
def book(request, pk):
	"""book page"""
	actual_book = Book.objects.get(id=pk)
	genre = Genre.objects.filter(genres=actual_book)
	print(genre)
	genre_ids = genre.values_list("id", flat=True)
	similars = (
		Book.objects.filter(genre__in=genre_ids).exclude(id=pk).order_by("?").distinct()[:4]
	)
...
def new_books(request):
	"""Look for new book in dtb on index page"""
